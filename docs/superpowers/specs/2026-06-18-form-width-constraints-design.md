...
Three-tier system for admin form pages:
...
| **Field Grid Row** | `grid-cols-1 md:grid-cols-2 lg:grid-cols-3` | Responsive column count that keeps individual inputs at 280–370px — the comfortable reading range. Same grid for all multi-field rows regardless of field count (2-field rows simply leave the 3rd column empty). |
| **Single-Field Row** | `max-w-[400px]` on the form item | A lone select or input doesn't stretch across the entire card width |
...
**Multi-field grid row — responsive column count:**
```html
<!-- Before -->
...
**Single-field row — capped width:**
```html
<!-- Before -->
<div class="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
  <el-form-item label="渠道"><el-select class="w-full" /></el-form-item>
</div>

<!-- After -->
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-4">
  <el-form-item label="渠道" class="max-w-[400px]">
...
- [ ] Field grid rows use `grid-cols-1 md:grid-cols-2 lg:grid-cols-3`
- [ ] Single-field rows have `max-w-[400px]` — don't fill the entire row
